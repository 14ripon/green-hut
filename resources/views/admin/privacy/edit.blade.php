            ...
            <div class="card lobicard lobicard-sortable" data-sortable="true" data-inner-id="x5dGz22KYW" data-index="4" style="display: block;">
                ...
                <div class="card-block p-3 basic-forms">
                        <form method="POST" enctype="multipart/form-data"
                              action="{{ route('admin-privacy.update', $page_content->id) }}">
                            @csrf
                            @method('PUT')
                            <div class="form-group">
                                <label for="name">Page Name</label>
                                <input class="form-control" id="name" name="page_name" value="{{$page_content->page_name}}">
                            </div>
                            <div class="form-group">
                                <label for="title">Title</label>
                                <input class="form-control" id="title" name="title" value="{{$page_content->title}}">
                            </div>
                            <div class="form-group">
                                <label for="message">Content</label>
                                <textarea name="content" id="editor1">{{ $page_content->content }}</textarea> <!-- /.Ck Editor -->
                            </div>
